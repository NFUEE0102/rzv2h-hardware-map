...
要安全地實跑，先替它排好一個**保證會發生的喚醒事件**再睡，別讓它睡到需要人工介入。最穩的做法是用 RTC 鬧鐘當喚醒源（RTC 落在永不斷電的 PD_AWO 域、深睡時仍走時，見本節上方電源域表與 06〈RTC〉）：
...
- **GPT／CMTW／WDT 側的 ELC 交叉引用**：GPT 的 ELC 事件在手冊 §5.7.6.1／§5.7.6.2、CMTW 在 §5.6.5.1、WDT→ELC 在 §5.4.4。這是 doc07 §13 列出的交叉引用指引，本群組筆記**未逐一開頁核對**原文，留給計時器群組（06）深講或後續查證時查用。
...
4. **GPT／CMTW／WDT 側的 ELC 交叉引用章節**（§5.7.6.1／§5.7.6.2、§5.6.5.1、§5.4.4）——轉引自 doc07 §13 的頁碼指引，留給 06 計時器群或後續深講查證。
