# 00 · Full-Board Hardware Resource Map
...
This chapter is a **folder**. This file (00) is the entry point, holding the chapter opener, the §4.1 full-board overview, and the chapter-end review/quick-reference table/data tables. The compute units are covered in depth in File 01; peripherals and interfaces are expanded unit by unit into the 02–08 group files, organized into eight groups; and the official documentation navigation guide is in File 09.
...
| [02-video-capture-codec-display.md](02-video-capture-codec-display.md) | The seven camera→scaling→codec→display units covered one by one in depth (CSI/CRU, ISU, ISP, VCD, VSP, LCDC/DU, DSI) |
| [03-audio-subsystem.md](03-audio-subsystem.md) | SSIU/SPDIF/PDM/SCU-ADMAC/ADG — all enabled at the SoC level, but the EVK has no physical codec broken out |
| [04-memory-and-storage.md](04-memory-and-storage.md) | Internal SRAM/Boot ROM/LPDDR4X controller/xSPI NOR/SDHI-eMMC |
| [05-system-backbone-interrupts-clocks-power-dma-event-link.md](05-system-backbone-interrupts-clocks-power-dma-event-link.md) | ICU+GIC-600/CPG/PMU/DMAC/ELC |
| [06-timing-system-timers-pwm.md](06-timing-system-timers-pwm.md) | SYC/GTM-OSTM/CMTW/GPT/POEG-PWM/WDT/RTC |
| [07-communication-and-sensing-interfaces.md](07-communication-and-sensing-interfaces.md) | SCIF/RSCI/RSPI/I²C/I3C/CAN-FD/CRC/GPIO/USB/GBETH+PTP/PCIe/ADC/TSU |
| [08-debug-and-security.md](08-debug-and-security.md) | CoreSight/TrustZone/Security IP — even "correctly absent" needs to be verified |
| [09-official-documentation-guide.md](09-official-documentation-guide.md) | What question each of the nine official documents in the folder answers, and how to instantly look things up in the 43.9 MB hardware manual using `_toc_full.txt` |

> **Section-number mapping**: where "4.1" appears in any file's body text it refers to this file's overview section, "4.2" refers to File 01, and "4.4" refers to File 09; the old "4.3 Peripherals and Interface Units at a Glance" has been expanded unit by unit into the 02–08 group files (the compute-unit positioning table is in Grouped Summary Table 1 in this file). Every ✅-marked verification step comes with a transcript filename, pointing to the on-board recordings under `live/` in the handbook folder (that's `../live/ch04*.txt`, reckoned from this folder).
...
- When you run into a register, spec, or deployment question, know straight away which chapter of which official document to flip to, instead of scrolling through a 4,800-page manual from cover to cover (File 09).

## What You'll Need

- **Hardware**: An RZ/V2H RDK dev board (SoC part number R9A09G057H44GBG) that can already boot and log in (either a serial console or SSH is fine — whatever state you reached at the end of the earlier chapters); this chapter doesn't require you to connect any additional peripherals.
...
- **Documentation**: The official documentation folder that you download from Renesas yourself and keep on **your PC/workstation** (not on the board; [09-official-documentation-guide.md](09-official-documentation-guide.md) introduces each document one by one). This handbook consistently refers to that folder by the relative path `../../reference-docs/` (reckoned from this folder) — swap that for wherever you actually keep it, or `cd` into it first before running the commands in File 09.
...
> - Hands-on steps are marked **📼 Per the live recording**: the steps and "expected output" are taken verbatim, unaltered, from the on-board live-recording logs of the probing and benchmark runs. A step marked **✅** has in addition been verified on the board, and carries a transcript filename pointing to the recording under `live/ch04*.txt`. A step marked **⏸** needs a heavy benchmark load or would interfere with a live service, so it rests on the live recording alone. This ✅ is a different mark from the ✅ in the resource tables' "Status" column (driver bound, directly usable) — read it according to context.
...
In the shipped device tree, the board vendor (here, Renesas's RDK dev board) sets only the nodes that **this particular board actually breaks out and intends to use** to `okay`, and leaves everything else `disabled`. The probing script scans the **first-level (directly under `soc`) nodes that carry a `status` property** under `/proc/device-tree/soc/` and counts them up separately, which is where you get:

> **45 `okay` (Enabled), 78 `disabled` (Disabled).** (Source: `06-hardware-resource-map.md`; measurement conditions same as this section's baseline. ✅ Verified on the board; transcript: live/ch04-dt-count.txt — a single line reproduces the same numbers: `for n in /proc/device-tree/soc/*/status; do tr -d "\0" < "$n"; echo; done | sort | uniq -c` → `78 disabled／45 okay`.)

The counting convention has three things worth spelling out up front (✅ Verified on the board; transcripts: live/ch04-dt-count.txt, live/ch04-cpu-periph.txt):
...
You'll see direct evidence of this "enablement comes in degrees" idea in a few places shortly: the OpenCVA vision accelerator is 🟡 (DT `okay`, memory all reserved, and **Renesas does provide a userspace library — it just isn't installed on this board yet**; how to install it is covered below). The two Cortex-R8s are 🟡 as well: the communication rings (vring — a cross-core shared-memory message ring) that remoteproc (remote processor — the framework Linux uses to load firmware for a coprocessor and manage its power state) needs are all reserved, and the only thing still missing is the R8 firmware, which has not been loaded. They're both "enabled," but neither is ready to just pick up and use the way the A55 or the NPU are.
...
|---|---|---|
| **123** (= 45+78) | Count of **nodes** directly under `/proc/device-tree/soc/` that carry `status` (scanning the whole tree is yet another ruler — the investigation documents record 136 nodes in total; only the 45/78 for nodes directly under `soc` is reproduced on the board, and the whole-tree figure has not been verified against a first-hand source) | `06-hardware-resource-map.md` |
| **52** | Count of **functional blocks** listed in the datasheet block diagram (Fig 1.1-1) (the datasheet is a degraded conversion, so this is provisional pending verification against a clean PDF, G2) | See `07-hardware-unit-usage-guide.md`'s citation of the datasheet |
...
There's one more thing worth separating out clearly up front: **"Disabled" does not equal "absent."** Most of the 78 `disabled` nodes are cases where the hardware exists in the silicon, but this particular board's device tree simply hasn't turned it on. There are two usual reasons: the pin is claimed by some other function's pin-mux (pin multiplexing — one physical pin is shared by several functions, and only one of them can be selected at a time), or the board never broke out the corresponding physical wiring. That's a different thing entirely from "this part number doesn't have this hardware at all." This distinction matters a lot when you want to "turn on something that's disabled" — see the note box next to the disabled list below. Running the other way, there's also the case of "present in the silicon, but the device tree never enabled it": the Mali-C55 ISP **is** on this H44 (**RZ/V2HP**) silicon (the hardware manual `r01uh1032` §1.1.2 Product Lineup, Table 1.1-1, p78 classifies part number R9A09G057H44GBG as RZ/V2HP and marks its ISP column verbatim as `Available (Mali-C55)`; corroborated by the WS125 RDK carrier board manual's component table, Page 11, where the main chip U1 is listed as "ISP&GPU"), but the running Linux has no node for it (details in the 02 group file). Note that the ISP has no device tree node at all, so it doesn't affect the "45/78" ruler either way. As for the hardware Security IP, that same SKU table (`r01uh1032` Table 1.1-1, p78 — the very table that establishes ISP = Available above) marks this part number's Security column verbatim as **N/A** — so it is **Not Populated** on this part (see the 08 group file).

### Learn to Read "Status" First: "Can't Find It" Has Six Different Meanings

The three symbols above, ✅/🟡/🟠, describe a single dimension: "does the `okay` node have a driver bound to it, and can you use it right away." Zooming back out — this chapter (including the 02–08 group files) uses the six status terms below to describe **the on-board status of any given hardware unit**. Get this concept straight first; it'll save you more debugging time than anything else here: **on this board, "I can't find some piece of hardware in Linux" is not one situation — it's six completely different situations**, and how you deal with each is worlds apart. Lumping them together is the single most common, most time-consuming trap for newcomers to this board.
...
| **Not Populated** | This particular silicon (R9A09G057**H44**) simply doesn't have this module — it belongs to a different part number (to determine which options this part number lacks, read the SKU columns of the hardware manual's Table 1.1-1, p78 — for example, hardware Security IP is marked N/A for this part, see 08) | Stop looking — use a different approach |

Here's one concrete example planted early; you'll run into variations of it in every group that follows. Some other tutorial tells you that `/dev/dma_heap` should have several heaps under it — system, cma — but on this board you find only a single root-only `linux,cma@58000000` (✅ Verified on the board; transcript: live/ch04-reserved-mem.txt; the inventory notes record no such directory at all, source `07-hardware-unit-usage-guide.md:187` — both accounts are given side by side, and the full breakdown is in the note box under "DMA, Interrupts, and Memory Reservations" further down). This isn't "broken": you simply get your contiguous buffers from the CMA (Contiguous Memory Allocator — a kernel-reserved mechanism for allocating large buffers that are contiguous in physical address space) region (`0x58000000`) instead. One of the teaching goals in each of the 02–08 group files is to train you to correctly sort every "can't find it" into the right one of the six slots above.

While we're at it, let's clear up two things that often cause confusion (source `07-hardware-unit-usage-guide.md:1,5,12`):

- **Why 49 is less than 52**: the datasheet block diagram lists 52 functional blocks, but the investigation document actually numbers only 49 unit paragraphs — because GTM and OSTM are actually the same 8-channel piece of hardware (see the 06 group file for details), and PWM isn't an independent peripheral either — it's produced via GPT + POEG (also see 06).
- **Taking stock of what this part number can do**: this board's SoC is **R9A09G057H44GBG** (H44 = the **RZ/V2HP** variant; the hardware manual `r01uh1032` §1.1.2 Product Lineup, Table 1.1-1, p78 classifies R9A09G057H44GBG verbatim as RZ/V2HP with its ISP column marked `Available (Mali-C55)`, and carries the Remark "The ISP is only present in the RZ/V2HP products." [p823]; corroborated by the WS125 RDK carrier board manual's component table, Page 11, where U1 is listed as "ISP&GPU"). It **does have** a Mali-G31 GPU (enabled); the **silicon does contain** a Mali-C55 ISP, but the current Linux device tree doesn't enable it (imaging goes through CRU pure-DMA instead); **hardware Security IP** (crypto engine/TRNG [true random number generator]/secure-boot accelerator) is **Not Populated** on this part number — that same SKU table (`r01uh1032` Table 1.1-1, p78) marks the Security column N/A, and on-board measurement likewise finds no hardware crypto interface anywhere in Linux. Details are in the 02 (ISP) and 08 (Security IP) group files respectively.
...
That line above, "45 Enabled / 78 Disabled," along with most of the tables in this section, was generated automatically by this one script. In a single pass it gathers device-tree status, driver bindings, clocks, power domains, IRQs, DMA, reserved memory, I²C/SPI/GPIO/CAN/USB/PCIe/Eth/MMC, v4l2, loaded modules, thermal, and more:
...
> 💡 **Tip**: the probing script is reproducible — on the same board with the same kernel version, running it again should give you the same result. Rather than typing out ②③'s commands by hand one at a time, it's better to generate a baseline log with the script up front, and only look up an individual item later when you suspect a unit's status has changed.

**② Look at the device tree's live state (the mechanism layer)**

The device tree's "live version" is mounted under `/proc/device-tree/`: each node is a directory, and if that node has a `status` property, there's a `status` file whose contents are the string `okay` or `disabled` (null-terminated). The most practical use of this is looking at the reserved memory regions: these nodes tell you which RAM has been assigned to hardware accelerators. Most of them carry `reusable`, so Linux can still borrow the memory through CMA until the hardware actually needs it; only the cross-core communication `vdev0*` regions carry `no-map`, and those are the genuinely hands-off ones:
...
`cat /proc/mtd` should list four partitions (this is where the boot image lives; ✅ Verified on the board, verbatim as follows; transcript: live/ch04-inventory.txt):
...
> **Symptom**: grep finds nothing at all (✅ Verified on the board; transcript: live/ch04-cpu-periph.txt — about 7.5 hours after boot, dmesg had only 1,419 lines left, and the earliest it retained were messages from around 26,863 seconds after boot, so the line above had already been pushed out).
...
> **Prevention/Remedy**: check it as early after boot as you can; or switch to an equivalent check that isn't subject to this limit — read the reserved region straight from the live device tree instead: `hexdump -C /proc/device-tree/reserved-memory/DRP-AI@240000000/reg` (✅ Verified on the board; transcript: live/ch04-reserved-mem.txt: `00 00 00 02 40 00 00 00  00 00 00 00 20 00 00 00`, i.e. start `0x240000000`, size `0x20000000` = 512 MB, which lines up exactly with the line above). Some boot messages can also be fished out with `journalctl -k -b` (this recovers `mali 14850000.gpu: Probed as mali0` — ✅ Verified on the board; transcript: live/ch04b-runtime.txt — but the reserved-region and DRP-driver-version lines aren't guaranteed to still be there).

**Confirm the accelerator nodes are all present**: when probing, several key accelerator/display device nodes should all exist (the numbers in parentheses are the major/minor device numbers; ✅ Verified on the board — all major/minor numbers match, and `/dev/drp1` measures as (234,1); transcript: live/ch04-inventory.txt):
...
| OpenCVA (DRP vision acceleration) | `c0000000.opencva` | Official library not installed | 🟡 | DT `okay`, memory reserved; **Renesas ships an OpenCVA library you can install** (see the note box below) |
...
(✅ Verified on the board: `drpai@16800000`, `drp1@18000000`, `gpu@14850000` and `opencva@C0000000` are all `status = okay`; the nodes `vsp@16480000`, `fcp@16470000`, `isum@16450000`, `vcp4@16400000`/`16410000` have **no `status` property at all** — by device tree convention this counts as enabled, which is also why they're not among the "45"; see the counting-convention explanation earlier. Transcript: live/ch04b-dt-status.txt.)
...
> ⚠️ **Note (OpenCVA isn't "no software, write it all yourself" — Renesas ships a library, you just have to install it)**:
> - **Scenario**: You see OpenCVA marked 🟡 in the table above and take it to mean "the hardware is there, but there's nothing at all in userspace — you'd have to compile your own operations into DRP circuits before you can use it."
> - **Correction**: The Renesas RDK official documentation (v1.1.1, Chapter 3 "OpenCV Accelerator") provides a **userspace library, OpenCVA**, at version **4.6.0** (matching Ubuntu 24.04 LTS's OpenCV 4.6.0). Once it's installed, **OpenCV functions on the supported list automatically reroute through the DRP when the conditions are met** — the calling code doesn't change, and you don't have to build any circuits yourself.
> - **Installation (the official procedure; not tested on this board by this handbook)**: remove the existing OpenCV first, then run the official script.
>   ```bash
...
>   ```
>   (Renesas notes that dependency warnings during the dpkg stage are expected, and that the script resolves them itself.)
> - **The 18 functions that get accelerated automatically**: `resize`, `cvtColor`, `cvtColorTwoPlane`, `GaussianBlur`, `dilate`, `erode`, `morphologyEX`, `filter2D`, `Sobel`, `adaptiveThreshold`, `matchTemplate`, `warpAffine`, `warpPerspective`, `pyrDown`, `pyrUp`, `FAST`, `remap`, `StereoSGBM`. (The official page misspells two of these as `wrapAffine`/`wrapPerspective`; OpenCV's actual function names are `warpAffine`/`warpPerspective` — searching the API for the official spelling will turn up nothing.)
> - **So why is this board still 🟡**: not because "there's no software," but because **this library hasn't been installed on this board yet**. Install it and it should become ✅.
> - ⚠️ **Read this handbook's own Chapter 3, §15.2 gate one, before you start**: the `apt remove` on the first line above will remove the system's existing OpenCV, and that chapter's `libshim/` is precisely what points `.so.409` at the system OpenCV 4.6 (`.so.406`). OpenCVA is also 4.6.0 with the same SONAME, so in theory the shim still holds after the swap, but **this combination has not been tested by this handbook** — back up a bootable card before you start.
> - **Source**: official documentation v1.1.1 `chapter-3/opencva/opencva.html`; separately, the Chapter 4 AI application page states that "the target board must use OpenCVA rather than the default OpenCV packages."
...
> **A clue that surfaced later (not enough to settle it, but it points at where to look)**: the Renesas RDK official documentation (v1.1.1, Chapter 3 "Video Codec Library") lists three usage restrictions — the **Video Codec Library is only available on the default image**, **Ubuntu Desktop may be incompatible**, and the GStreamer plugins on the board **come from the apt repositories rather than a Renesas-customized build**. That lands squarely on the "the two measurements had different image states" guess above: **on one and the same board, a different system state (whether a desktop environment is installed, which build the plugins came from) can genuinely produce a different answer for the hardware encode path**. This handbook has no record of the image state at the time of either measurement, so it **still draws no conclusion** about the discrepancy — the restriction is simply recorded here so you can check your own environment first when troubleshooting.
> **Prevention/Remedy**: on your own board, don't assume the hardware encoder is necessarily ready. First use `gst-inspect-1.0 omxh265enc` to confirm the OpenMAX plugin is actually there and can be probed, then decide whether to go the hardware or software encoding route. (✅ Verified on the board: both `gst-inspect-1.0 omxh265enc` and `omxh264enc` list the plugin, with Klass marked `Codec/Encoder/Video/Hardware` — that confirms the "plugin can be probed" side of things; whether actual recording goes through hardware or software still comes down to testing your own pipeline. Transcripts: live/ch04-cpu-periph.txt (omxh265enc), live/ch04-followup.txt (omxh264enc).)
...
|---|---|---|
| I²C | ✅ 4 enabled | `i2c-3` = display/HDMI/system (ADV7535 @0x3d); `i2c-4` = camera (TEVS @0x48) + IMU (LSM6DSO16IS @0x6a); `i2c-8` = clock generator (VersaClock 3S, part number 5L35023B-616NLGI8, U30; WS125 RDK carrier board manual Page 11 BOM; the @0x69 address is as recorded in the investigation documents and was not verified by measurement, since i2c-8 is never scanned) / reserved; `i2c-9` = free to use. (buses 2/5/6/7 disabled) |
...
| USB | ✅ | Physical ports: 2× USB3.2 Gen2 Type-A (CN2, backward-compatible with USB2) + 1× micro-B (UART serial/function, **not** a data port); there is no separate USB 2.0 host connector (board manual §3.6/§3.7). What `lsusb` shows — 2× USB2.0 + 2× USB3.0 root hubs — is 2 xHCI controllers each carrying a USB2/USB3 companion, corresponding to those 2 Type-A ports, **not** 4 physical ports; this column varies with what's plugged in — in the recorded state a USB wireless adapter was attached (✅ Verified on the board; transcript: live/ch04b-runtime.txt) |
| PCIe | ✅ | Gen3 Root-Complex (`13400000.pcie`) enabled; no endpoint currently attached |
| Ethernet | ✅ | `end0` (GbE, `dwmac`) UP — the interface is capable of 1 Gbps, actual link speed depends on the far end (on the board `/sys/class/net/end0/speed` reads `100`; ✅ Verified on the board; transcript: live/ch04b-runtime.txt); the 2nd GbE (`15C40000`) disabled (✅ Verified on the board as DT `disabled`; transcript: live/ch04b-dt-status.txt) |
...
(✅ Verified on the board: `ethernet@15C40000`, `mmc@15c10000`/`15c20000` (on-board eMMC controller) and `csi22@16020400`/`csi23@16030400` are all `disabled`; of GPT's 16 `gpt@…` nodes, only `gpt@13010000` is `okay`, the other 15 are `disabled` — matching each of this section's tables point for point. Transcript: live/ch04b-dt-status.txt.)
...
**Some of this doesn't need a dtb rebuild at all — Renesas already packaged it as an overlay, and one config line switches it on or off.** "Modify the device tree" above sounds like rebuilding the whole dtb (that route is in Chapter 2, Appendix 2A), but this board has an official **device tree overlay** mechanism: `/boot/uEnv.txt` holds a handful of switches — uncomment one, reboot, and it takes effect; the `.dtbo` files already ship with the system and don't need compiling (Renesas RDK official documentation v1.1.1, Chapter 1 Advanced Setup → "Device Tree Overlay").

**The factory `uEnv.txt` actually carries six switches** (📼 per the live recording of `cat /boot/uEnv.txt` on the board; the official documentation lists only the first three — the three camera overlays that follow aren't mentioned there):

| Setting line in `/boot/uEnv.txt` | Factory default | Overlay loaded | What it does |
|---|---|---|---|
| `enable_overlay_can` | **`1` (enabled)** | `rzv2h-rdk-1.0-can.dtbo` | CAN interface |
| `enable_overlay_audio_codec` | commented out | `rzv2h-rdk-1.0-audio-codec.dtbo` | External audio codec. **Enabling it disables micro-HDMI audio** (see the 03 file) |
| `enable_overlay_spi` | commented out | `rzv2h-rdk-1.0-ext-spi.dtbo` | External SPI (`rsci_spi0`; `P50` = MOSI, `P51` = MISO, `P52` = SCK, `P53` = SS [slave mode]) |
| `enable_overlay_imx219` | `0` | `rzv2h-rdk-1.0-imx219.dtbo` | Raspberry Pi IMX219 camera (CSI20/CN5) |
| `enable_overlay_tevs_cam0` | `0` | `rzv2h-rdk-1.0-tevs-cam0.dtbo` | TechNexion TEVS camera on **CN5** (RIIC `i2c@14401400` → csi20). ⚠ **This official overlay is `data-lanes = <2>` (2-lane)** |
| `enable_overlay_tevs_cam1` | `0` | `rzv2h-rdk-1.0-tevs-cam1.dtbo` | TechNexion TEVS camera on **CN6** (RSCI `i2c@12802400` → csi21, automatically setting the factory `ov5645_csi21` to `disabled` to make room). `data-lanes = <4>` |

> ⚠️ **Note (the official `tevs-cam0` overlay is 2-lane, incompatible with this handbook's 4-lane route)**: Scenario — you see there's a ready-made TEVS overlay and want to skip the dtb rebuild in Chapter 2, Appendix 2A. Symptom — the camera probes, but 1080p can't hit full frame rate and the bandwidth doesn't add up. Cause — decompiling `rzv2h-rdk-1.0-tevs-cam0.dtbo` shows it declares `data-lanes = <0x02>` with the endpoint also `<0x01 0x02>`, whereas CN5 is actually routed as a full 4 lanes; **this handbook takes the 4-lane route of editing the base dts directly** (Appendix 2A), and the two are mutually exclusive. Prevention — on this handbook's route, keep both `enable_overlay_tevs_cam0` and `enable_overlay_tevs_cam1` at **`0`**. (📼 per the `dtc -I dtb -O dts` decompilation recording.)

> ⚠️ **Note (`uEnv.txt` — only the overlay lines may be touched)**: Renesas states explicitly, "**Only comment or uncomment device tree overlay settings in this file. Do not modify any other entries, as doing so may cause unpredictable boot behavior.**" — this same file is also where U-Boot gets its kernel/dtb load paths (`fdtfile` should be `rzv2h-rdk-ver1.dtb`), so breaking another line hits booting directly. **To switch one of these existing features on or off, change only the comment character on that one line and leave everything else alone**; back up a copy of `uEnv.txt` before you edit.

> ⚠️⚠️ **Note (to *add* a new overlay, adding `enable_overlay_X=1` alone does nothing)**
> - **Scenario**: you build your own `.dtbo`, drop it into `/boot/dtb/renesas/overlays/`, add a line `enable_overlay_myfeature=1` to `uEnv.txt` following the pattern in the table above, and reboot.
> - **Symptom**: nothing happens at all — no new nodes anywhere in the live device tree (`/proc/device-tree/`), no `Applying DT overlay` line in the boot messages, **and no error either**.
> - **Cause**: `enable_overlay_X` is only a **flag variable**; the actual work happens in two other layers. Each switch in `uEnv.txt` is paired with an `apply_ov_<name>` U-Boot script that does the `ext4load` + `fdt apply`, and those `apply_ov_*` scripts **only get called if they're listed in the `mmc_ovfdt` execution sequence**:
>   ```
...
>   ```
>   An `apply_ov_*` that `mmc_ovfdt` doesn't name will never run.
> - **Prevention/Remedy**: adding an overlay means doing three things **together** — ① drop in the `.dtbo`; ② define `enable_overlay_<name>` and its matching `apply_ov_<name>`; ③ insert `run apply_ov_<name>` into the `mmc_ovfdt` sequence. This goes beyond Renesas's "only comment or uncomment" boundary and counts as a knowingly risky modification, so **back up `uEnv.txt` before you touch it**, and have Chapter 2 Appendix 2A's "reroute once at the U-Boot prompt without `saveenv`" recovery technique ready.
> - **Criterion**: after the edit and a reboot, the boot messages should show `Applying DT overlay: <yours>.dtbo`; if that line isn't there, it never got called.
>
> (📼 per the verbatim live recording of `uEnv.txt` on the board.)

> 💡 This also draws the line between the two routes: **whatever an overlay covers goes through `uEnv.txt`** — one line, reversible, no toolchain involved; **whatever an overlay doesn't cover** (the eMMC and 2nd GbE above, say, or this handbook's 4-lane camera dts change) is what needs the dtb-rebuild route in Chapter 2, Appendix 2A. Renesas currently provides **no** overlay for PCIe, GPIO, or multi-OS; the camera overlays do exist (imx219/tevs-cam0/tevs-cam1) but, as described above, their specs don't match this handbook's route.
...
> **Current status (two accounts given side by side)**: the inventory notes record no `/dev/dma_heap` at all (source `07-hardware-unit-usage-guide.md:187`); on the board it **does exist** (✅ Verified on the board; transcript: live/ch04-reserved-mem.txt), containing a **single** heap: `/dev/dma_heap/linux,cma@58000000` (permissions `crw-------`, root-only) — the name itself points straight at the CMA region, with no system heap or other heaps alongside it.
...
**Devices with currently active interrupts.** Watch them with `watch -n1 'cat /proc/interrupts'`. The registered interrupt sources include `arch_timer`, `end0` (network), `rzg2l_cru` (camera), `drp` (DRP1)/`drpa` (DRP-AI), `canfd`, `spi`, `serial`; which counters are ticking, and how fast, depends on who's actually active at that moment (✅ Verified on the board; transcript: live/ch04-followup.txt): the ones ticking are `arch_timer`, `end0`, `rzg2l_cru`, `drpa mac_nmlint` and `serial`, while `canfd` and `spi` are registered but sit at a count of 0 (no load attached means no activity). This list is itself a live inventory of "what's active right now."

### Memory Reservation Map (carveout)

Not all 16 GB of physical memory is handed to Linux as general-purpose RAM. At boot time, a batch of **carveouts (reserved regions)** is already sliced off for hardware accelerators, display, camera capture, and cross-core communication. Two kinds of reservation need to be told apart here (✅ Verified on the board; transcript: live/ch1-p2-resmem.txt). Most of the accelerator/display/camera regions carry the **`reusable`** flag: they aren't walled off, and until the corresponding hardware actually needs them, Linux's general-purpose allocator can still borrow that memory (via CMA/movable pages). That is why `MemAvailable` comes out higher than "total RAM minus every carveout" would suggest, and why the measured `CmaTotal` of roughly 1.7 GB is exactly this reclaimable batch. Only the cross-core communication **`vdev0*`** regions (the OpenAMP rings and buffers for the R8/M33) carry **`no-map`**, and those are the ones Linux never maps and genuinely cannot touch. The table below is the reservation map for the whole board, with addresses preserved verbatim so you can cross-check them against `hexdump .../reg` and `/proc/iomem` output. Every start address and size has been checked item by item with `hexdump -C /proc/device-tree/reserved-memory/*/reg` (✅ Verified on the board; transcripts: live/ch04-followup.txt, live/ch04-inventory.txt). Note that vring is actually `vdev0vring0@0x43000000` + `vdev0vring1@0x43050000`, 320 KB each, 640 KB combined:
...
(✅ Verified on the board: `MemTotal` is still `15565904 kB`; `MemFree`/`MemAvailable` float with whatever's running at the moment — one reading gives `13045444`/`14554288` kB — so when comparing against the baseline, only `MemTotal` counts; transcript: live/ch04-inventory.txt.)
...
> 💡 **Tip**: the table above is a condensed "main PLL → subsystem" summary; the frequency values for each PLL are taken from the investigation-document inventory and have not each been checked against a first-hand manual. The complete, first-hand-backed table of all 11 PLLs (including what each one serves) is in the 05 "System Backbone" group file (source r01uh1032 §4.4.1, p620–621). To see the **actual** frequencies across the whole clock tree and each clock's enable count, run `sudo cat /sys/kernel/debug/clk/clk_summary | less`, and go by what you find on the board.
...
| USB (2× USB3.2 Gen2 Type-A, USB2-compatible; the micro-B is UART, not a data port) | Extra sensors, controller USB (e.g., a flight controller), expansion dongles |
...
1. ✅ **Reproduce the node count** (verified on the board; transcript: live/ch04-dt-count.txt): a single line reproduces it — `for n in /proc/device-tree/soc/*/status; do tr -d "\0" < "$n"; echo; done | sort | uniq -c`, which should give `78 disabled／45 okay`. (The full script `SUDO_PW=<pw> bash hw_resources.sh <output directory>` needs a sudo password and scans I²C, so it is not part of this check — 📼 per the live recording.)
   *Pass criterion*: numbers match → you've reproduced this section's core finding.
2. ✅ **Check the reserved memory regions** (verified on the board; transcripts: live/ch04-inventory.txt, live/ch04-reserved-mem.txt): `ls /proc/device-tree/reserved-memory/` shows a batch of reserved-region nodes; `hexdump -C /proc/device-tree/reserved-memory/DRP-AI@240000000/reg` should match start `0x240000000`, size `0x20000000` (= 512 MB = 524288 KiB). (The `dmesg` line is only findable shortly after boot — it's a ring buffer, see the note box earlier.)
   *Pass criterion*: the address and 512 MB (524288 KiB) match up.
3. ✅ **Are the accelerator nodes all there** (verified on the board; transcript: live/ch04-inventory.txt): `ls -l /dev/drpai0 /dev/mali0 /dev/media0 /dev/video0 /dev/dri/card0`.
   *Pass criterion*: all five nodes are present (cross-check major/minor numbers against the list above).
4. ✅ **I²C** (verified on the board; transcript: live/ch04-inventory.txt): `ls /dev/i2c-*` should list the enabled I²C buses (this section's baseline is 4: i2c-3/4/8/9).
5. ✅ **Boot partitions** (verified on the board; transcript: live/ch04-inventory.txt): `cat /proc/mtd` should show `mtd0 "bl2"`/`mtd1 "fip"`/`mtd2 "env"`/`mtd3 "test-area"` (full verbatim output earlier above).
6. 📼 **DMA channel count** (needs sudo; per the live recording): `sudo cat /sys/kernel/debug/dmaengine/summary` should count out to about 32 channels (recall the earlier point about the datasheet saying 80, Linux seeing 32 — not a fault).
7. ✅ **RAM capacity** (verified on the board; transcript: live/ch04-inventory.txt): the first line of `cat /proc/meminfo`, `MemTotal`, should read `15565904 kB` (about 14.8 GiB).
...
- **Whether something's enabled is decided by the device tree**: only `status = okay` gets probed by the kernel; "45 Enabled / 78 Disabled" counts **DT nodes directly under `soc` that carry `status`** (scanning the whole tree is yet another ruler — the investigation documents record 136 nodes in total, a figure not reproduced on the board; nodes with no `status` property count as enabled), and it has a different denominator from "52 functional blocks" or "49 numbered units" — you can't convert between them or subtract one from another.
- **"Enabled" comes in degrees**: DT `okay` ≠ ready to use right now (OpenCVA's official library isn't installed, the R8s are awaiting firmware); "can't find it" has six meanings — Enabled / Partially Enabled / Present · Not Exposed to Linux / Reserved / Disabled / Not Populated — and each calls for a different response.
- **Compute is almost entirely on, and the interfaces leave a row of open slots**: already claimed are GbE (`end0`), the SD card, the camera (CSI20), DRP-AI3/DRP/GPU/VCD; free are CAN-FD ×2, 2× USB3.2 Type-A, PCIe Gen3 RC, the 2nd CSI lane, SPI/I²C bus9/ADC, and two Cortex-R8s awaiting firmware.
...
- **DRP-AI3 is the decisive advantage for perception**: YOLOX-nano pure inference at 15.26 ms (CPU 4-thread: 145.6 ms, i.e. 9.7× faster measured against the 15.0 ms pure-inference p50), freeing up all four A55s entirely. But it's a **single, serialized NPU**: running multiple models means time-slicing, and total FPS = 1 ÷ the sum of the latencies. Pure inference itself has no fixed floor (small models such as mobilenetv2 at 1.34 ms and resnet50 at 4.19 ms get down to 1.3–4.2 ms); end-to-end comes out longer than pure inference because of the pre- and post-processing stages around it (roughly 6.86 + 1.62 ms for YOLOX-nano@416), not because every frame pays a fixed startup floor.
...
- **What you can't reach from Linux on this H44 part number**: the Mali-C55 ISP (present in the silicon, but not enabled in the device tree — imaging goes through CRU pure-DMA), hardware Security IP (the hardware manual's Table 1.1-1, p78 marks the Security column N/A, so it is Not Populated on this part; the board likewise has no `/dev/tee` or any other crypto interface); the `/sys/class/pwm` directory exists but is empty of any pwmchip; `/dev/dma_heap` has only a single root-only CMA heap (see §4.1 for the discrepancy between the two records) — "correctly absent," "correctly empty," and "in the silicon but not enabled" all need to be verified.
...
A list of the commands commonly used for on-board probing and verification (for detailed usage and expected output, see the file indicated in the "Appears In" column: "this file §4.1" = the sections of this file; "File 01" = [01-compute-units.md](01-compute-units.md); "File gN" = the corresponding group file; "File 09" = [09-official-documentation-guide.md](09-official-documentation-guide.md)):
...
| `cat /proc/mtd` | xSPI NOR's MTD partitions (bl2/fip/env/test-area) | This file §4.1, File 04 |
...
| `cat /sys/class/thermal/thermal_zone0/temp` (same for zone1) | Chip temperature (m°C; idle around ~34–36 °C) | File 01 |
...
| `gst-inspect-1.0 omxh265enc` | Confirm whether the VCD hardware-encode plugin is available | This file §4.1, File 02 |
| `ls /dev/i2c-*` | The enabled I²C buses (i2c-3/4/8/9) | This file §4.1, File 07 |
| `i2cget -y 4 0x6a 0x0f` | Read the IMU WHO_AM_I (expected `0x22`; do not scan i2c-8) | File 07 |
| `gpioset $(gpiofind PA2)=0` | Release the CAN transceiver from standby (required before bringing up can0) | File 07 |
| `ip link set can0 up` | Bring up the CAN interface (do the previous step first) | File 07 |
| `cat /sys/power/state` | Supported suspend modes (freeze mem disk) | File 05 |
| `cat /proc/asound/cards` | Sound cards (rcarsound) | File 03 |
| `ls /dev/tee* 2>/dev/null; ls -la /dev/dma_heap/; ls -A /sys/class/pwm \| wc -l` | Three-in-one check: TEE should not exist, dma_heap should have only linux,cma, pwm should be 0 | Files 05/06/08 |
| `grep -E "^[0-9]+	SECTION " ../../reference-docs/_toc_full.txt` | The starting pages of the hardware manual's 10 SECTIONs | File 09 |
| `grep "<unit name>" ../../reference-docs/_toc_full.txt` | Look up which chapter/page a given unit is on in the manual | File 09 |

## Chapter Measured-Data Tables

This rolls up the measured data from across the whole chapter. **Common measurement baseline** (applies to the "Conditions" column wherever not otherwise noted): on-board `ubuntu@<board IP>`, kernel `6.10.14-arm64-renesas` (SMP PREEMPT, not PREEMPT_RT), Ubuntu 24.04.4 LTS aarch64, CPU governor locked to `performance` (1.7 GHz). The original logs are in `../../assets/compute-benchmark-20260621/results/` and `../../assets/hardware-investigation-20260621/`. The Hz tables from the capability-ceiling derivations (EKF/MPC/FFT/per-model FPS) are **ESTIMATE extrapolations, not measurements**, and are not included in this table — see "Capability-Ceiling Derivations" in [01-compute-units.md](01-compute-units.md). Of these, the resource-inventory category (the 45/78 node count, the full carveout table and the 512 MB DRP-AI reservation, the I²C buses, the IMU WHO_AM_I, MemTotal, the MTD partitions, the accelerator device nodes) is verified on the board (transcripts: live/ch04*.txt, noted item by item in each section). The benchmark category (CPU/memory/storage/NPU/GPU/real-time/thermal) involves heavy loads, so it rests on the live recording.

**Resource Inventory**

| Item | Measured Value | Conditions | Source |
|---|---|---|---|
| device-tree node count | 45 `okay`/78 `disabled` (nodes directly under `soc` carrying `status`; the whole-tree 52/84 figure is from the investigation documents and was not reproduced here) | the 45/78 is verified on the board with a one-line command; the whole-tree 52/84 has not been verified against a first-hand source | 06-hardware-resource-map.md:20 |
...
| CPU comparison | 4-thread 145.6 ms (6.9 FPS)/1-thread 422 ms (2.4 FPS); speedup 9.7×/28.1× (both against the 15.0 ms pure-inference p50) | onnxruntime fp32 | 05b_npu_cpu.txt, 05_npu_drpai.txt:45 |
...
- `../../reference-docs/r01an7723ej0400-rzv2h-rzv2n-awo-example-program-startup-guide.pdf` — the AWO example program startup guide (CM33 firmware/Yocto deployment; key points and gotchas in File 09).
...
- `../../reference-docs/r01us0653ej0202-rzv2h_rzv2n_GStreamer_UME.pdf` — GStreamer UME (the official software manual for GStreamer pipelines and OMX codec elements under Linux); consult it for "how to build the pipeline, which element takes the hardware path" (see File 09 for details).
- `../../reference-docs/REN_WS125V2HRDKREFZ_MAH_20260323.pdf` — the WS125 RDK **carrier board** hardware manual (consult it for board-level questions like connector pinouts, the camera connector, DIP switches); note that although the extension is `.pdf`, it's actually a ZIP package of page images + text (see File 09 for details).
