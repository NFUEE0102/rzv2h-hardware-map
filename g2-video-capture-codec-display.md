...
This is a **group reference file** under the "Full-Board Hardware Resource Map" chapter — it lays out, one by one, the seven hardware units that make up the entire video pipeline on the RZ/V2H (part number R9A09G057H42), from "the camera turning light into data" all the way to "the picture showing up on a screen." This chapter's file 00 (4.1 Overview) and file 01 (4.2 Compute Units) give you the whole-board quick-orientation view; what you get here is **the mechanism, Linux interface, capability boundaries, and decision rules for each individual unit** — when you need to decide "how should the camera be hooked up, which piece of hardware should do the scaling, should you use hardware encoding, how does the picture get output to a screen," the answers are here.
...
| [ISP Mali-C55](#3-isp-mali-c55-not-populated-on-this-chip) | Hardware image signal processor (debayer / white balance / HDR) | **Not Populated** (an RZ/V2HP-exclusive option; the H42 silicon doesn't have this hardware) |
| [VCD (H.264/H.265 Hardware Codec)](#4-vcd-h264h265-hardware-codec) | A hardware video codec, bidirectional H.264/H.265 | Two accounts side by side: OMX plugin exposed / bench testing once observed it running x264 CPU software encoding (go by `gst-inspect-1.0` on your own board) |
...
There are two forks in this path worth flagging up front: **(1)** the spot in this chain where "there should be a hardware ISP helping the camera do demosaicing/white balance" (between the camera and the ISU) is **empty** on this part number — the ISP Mali-C55 is Not Populated, so that stage of work has to be handled instead by the CPU/DRP/DRP-AI software, or by the camera module itself (see Section 3). **(2)** Both the ISU and the VSP can do "format/size reshaping," but they divide the labor differently: pure scaling/format conversion goes through the ISU, which is lighter-weight; the VSP only gets called in when multi-layer blending, overlay compositing, or LUT correction is involved (see the decision rules in Sections 2 and 5).
...
> **Actual on-board situation (two records presented side by side, kept faithfully as-is)**: in the device tree, only **the two nodes CSI20/CSI21 are `okay`** — CSI22/CSI23 are `disabled` (✅ confirmed on-board 2026-07-18, transcript: live/ch04b-dt-status.txt) — in other words, the silicon supports 4 channels, but this board's device tree only turns on 2 of them. As for how many capture nodes get exposed, the two records don't fully agree: doc07 §39 records bench testing that exposed `/dev/video0`, `/dev/video1`, and `/dev/media0`; while the 2026-07-18 on-board re-check only found `/dev/video0` and `/dev/media0`, with no `/dev/video1` in sight (transcript: live/ch04b-runtime.txt). Before you start working, go by whatever `ls /dev/video* /dev/media*` actually shows on your own board at the time.
...
- **Channels and lanes**: 4 channels, each channel selectable as 1/2/4 lanes (a lane = a pair of differential signal lines; more lanes means higher total bandwidth). The datasheet specifies a maximum bandwidth per lane of **2.1 Gbps**, with a supported throughput ceiling of "**4K RAW12 60 fps**" (`r01ds0429-datasheet.md` line 349–353).
...
- **For functionality that depends on image statistics, pick the right channel**: take an application that needs auto-exposure/auto-white-balance as an example — this kind of feature consumes the image statistics the CRU outputs, and only CRU0/CRU1 have statistics output (AXI-SD). At the same time, keep in mind — this SoC's silicon **has no hardware ISP** (see the next section), so once you have the statistics data, you have to feed it to your own software or DRP algorithm yourself; the hardware won't finish the auto-exposure calculation for you.
...
## 3. ISP Mali-C55 (Not Populated on This Chip)
...
**But this unit is Not Populated on the chip this handbook is about (R9A09G057H42, i.e. RZ/V2H).** The datasheet explicitly marks the Mali-C55 ISP as "(option\*) \*RZ/V2HP only" — it's an option exclusive to **RZ/V2HP**, another part number in the same family; the H42 silicon simply does not have this piece of hardware at all (doc07 §7, citing datasheet Table 1.3-2/1.4).
...
You don't — because the hardware doesn't exist:

- **No device node, no driver binding**: this board has no v4l2 ISP subdevice path like `rzg2l-isp` (doc07 §7).
- **Camera capture itself is unaffected**: the CRU/CSI-2 capture path works as normal — `/dev/video0` (along with `/dev/video1`, in doc07 §7's record) can still capture RAW images fine (doc07 §7) — ✅ on the 2026-07-18 on-board re-check, the capture path actually exposed was `/dev/video0` and `/dev/media0` (transcript: live/ch04b-runtime.txt). All that's missing is the stage where "hardware turns the RAW into a finished image for you."
...
The following specs are **for reference only — they don't apply to this chip** (they're listed purely so you know that "switching to the RZ/V2HP part number gets you this extra unit," so you have that in mind when choosing a part): Video Input max 4096×2160, AXI3 bus 256-bit@400MHz(max); Video Output max same size, AXI3 128-bit@630MHz(max); output supports 13 color formats (6 RGB, 7 YUV); input data types RAW8/10/12/16/20, CFA pattern (color filter array arrangement) supports only 2×2 RGGB Bayer (9.8.1.1.1, p4688). None of these numbers hold on the H42 board — the hardware doesn't exist.

### When You'd Actually Use This

- **Check which silicon part number you actually have first**: mechanically, only RZ/V2H**P** has this hardware ISP available; RZ/V2H (what this handbook is about) does not. Any functionality that depends on a hardware ISP (folding auto-exposure statistics into the pipeline, hardware demosaicing, hardware HDR compositing) has to be handled instead by software/DRP processing on RZ/V2H, or you need to pick a camera module with a built-in ISP instead (one where the camera does its own ISP work and outputs already-processed YUV, rather than RAW).
- **This rule has nothing to do with the application domain**: take industrial defect inspection, robot navigation, or aerial imaging as examples — as long as the camera module you picked **outputs RAW Bayer data directly**, you'll have to add a software or DRP demosaicing/white-balance stage on RZ/V2H, no matter which of these it is. Conversely, if the camera module already has a built-in ISP (for example, the TEVS-AR0234 camera module that this handbook's reference board uses), this limitation doesn't affect you — the deciding factor is **the camera module's spec sheet**, not the SoC's spec.

> Endnote (Sources): the official hardware manual `r01uh1032` §9.8 Image Signal Processor (ISP) (p4688–4689, overview; Not Populated on this part number, for reference only. The Manual explicitly states this chapter is a simplified version, with the complete content in the Additional Document, which isn't included). Other documents: datasheet `r01ds0429` Table 1.3-2/1.4 (HP-SKU-exclusive).
...
> - **Symptom**: One record (from the probing-script's viewpoint) shows the VCD driver already bound, with the OpenMAX plugins `omxh264enc`/`omxh265enc` already exposed (Source: `06-hardware-resource-map.md:35,41`, `07-hardware-unit-usage-guide.md:133`); but another bench-test record focused specifically on codec testing observed that what was actually running at the time was x264's **CPU software encoding** path (Source: `05-compute-benchmark.md:204-205`). The two don't agree on whether the hardware encoder can be used directly right now.
...
> - **Prevention/Handling**: don't assume the hardware encoder is ready to go by default. First confirm with `gst-inspect-1.0 omxh265enc` that the plugin really is there and can be probed, then check whether your actual pipeline really does connect to `omxh265enc` rather than falling back to software x264 — only then decide which path to take. (✅ Re-run on the board on 2026-07-17: `gst-inspect-1.0 omxh265enc` and `omxh264enc` both list the plugin, with Klass marked `Codec/Encoder/Video/Hardware` — this falls under the "plugin can be probed" case; whether an actual recording runs through hardware or software still comes down to testing your own pipeline in the moment. Transcript: live/ch04-cpu-periph.txt (omxh265enc), live/ch04-followup.txt (omxh264enc).)

### Key Capabilities & Limits

- **Supported profiles/levels**: H.264/AVC — constrained baseline/main/high profile, all at level 4.2; H.265/HEVC — main profile, level 5 (9.6.1, p4683). (Profile/level are the grading system within a video standard that defines "which compression tools are used, and how large a resolution and bitrate (also called bit rate) it supports.")
...
- → **RPF** (Read Pixel Formatter — up to 2 units, RPF0/RPF1; handles unpacking the pixel format, color-space conversion, color-depth conversion, color keying, ROP operations, alpha multiplication, and OSD processing)
...
- **Channels and bandwidth**: 1 channel, up to 4 data lanes, maximum bandwidth per lane of **1.5 Gbps**, capable of supporting up to Full HD (1920×1200)@60fps (RGB888) at most (9.5.1.1 Overview, p4548).
...
| ISP Mali-C55 | 9.8 Image Signal Processor (ISP) (p4688; Not Populated on this part number) | — (Not Populated) |
