...
```text
┌─────────────────────────────────────────────────────────────────────────────────────────────────────────────────────┐
│  RZ/V2H (R9A09G057H42) Heterogeneous Compute Units                                                                  │
├─────────────────────────────────────────────────────────────────────────────────────────────────────────────────────┤
│                                                                                                                     │
│  ┌──────────────────────┐                                                                                           │
...
**Why "has dotprod and fp16, but no aes/sha" matters.** `asimddp` (INT8 dot product) lets the A55 do INT8 dot products relatively efficiently, which helps the CPU fallback path for quantized neural networks (quantization = compressing weights/activations from fp32 down to low-bit integers like INT8, trading precision for a smaller model and faster NPU/integer math); `fphp`/`asimdhp` (fp16) gives half-precision floating point hardware support. But the instruction set **does not have** ARMv8 Crypto Extensions — meaning AES and SHA can only run as software implementations, with throughput pinned to the tens-of-MB/s range (the benchmark section later gives you exact numbers). This isn't a defect — it's a characteristic of this H42 part number: the datasheet explicitly notes that the crypto extension is "security variant products only," and this part simply doesn't have it (Source: 07-hardware-unit-usage-guide.md:19, datasheet.md:180).
...
> ⚠️ **Warning**: Regarding the A55's **boot frequency and voltage** — the two sources don't agree on the numbers, so know this going in before you write any code. The on-board DIP switch `BOOTPLLCA[1:0]` determines the boot point: OFF/ON (default) = 1.7 GHz@0.9V; ON/OFF = 1.8 GHz@0.9V; for the OFF/OFF setting, the quick-reference notes say "1.0 GHz@0.8V," while the datasheet's rated value says "1.1 GHz@0.8V" (Source: 04-hardware-quickref.md:32-35, datasheet.md:148-157). **Symptom**: if you expect the low-power setting to run at some exact frequency based on one document, the measured value may be off by one notch. **Cause**: the two documents don't agree on the nominal value for the same setting — that's just how the sources are, and this handbook won't arbitrate between them. **Prevention**: don't hard-code "the exact frequency of the low-power setting" into your program; to find out which frequency is actually in use, read `scaling_cur_freq` after boot using the command below. Every CPU measurement in this handbook was taken at the **default 1.7 GHz, governor locked to performance** — that part is unambiguous.
...
**Current on-board status: exposed via remoteproc, but offline by default with firmware missing.** Unlike R8, M33 **is visible** in Linux: `/sys/class/remoteproc/remoteproc0` is it, named `cm33`, default state `offline`, with the firmware field showing `rproc-cm33-fw` (✅ re-run on-board 2026-07-17: all three values matched verbatim; transcript: live/ch04-cpu-periph.txt [name/state], live/ch04-followup.txt [firmware]) (the actual filename is `rzv2h_cm33_rpmsg_linux-rtos_example.elf`). The reserved memory it uses is rsctbl@0x42f00000, vring@0x43000000, vdev0buffer@0x43200000 (Source: 07-hardware-unit-usage-guide.md:49-50).
...
**✅ Step 1 (re-run on-board 2026-07-17): Confirm the four A55 cores and their clock.**

```bash
lscpu | grep -E 'Model name|CPU max|CPU min|Core\(s\)'
```

Expected output (verbatim, ✅ re-run on-board 2026-07-17, transcript: live/ch04-followup.txt; also see 00_inventory.txt:29-42):
...
**✅ Step 3 (re-run on-board 2026-07-17, transcript: live/ch04-followup.txt): Confirm the instruction set extensions — dotprod/fp16 present, aes/sha absent.** This determines whether you can count on hardware crypto.
...
**✅ Step 4 (re-run on-board 2026-07-17, transcript: live/ch04-cpu-periph.txt): Confirm M33's remoteproc status (offline by default is normal).**
...
**✅ Step 5 (re-run on-board 2026-07-17, transcript: live/ch04-inventory.txt): Confirm all the accelerator device nodes are present.**

```bash
ls -l /dev/drpai0 /dev/mali0 /dev/drp1 /dev/media0 /dev/video0 /dev/dri/card0
```

Expected: all six nodes present. Their measured major/minor device numbers are `/dev/dri/card0`(226,0), `/dev/drpai0`(511,0), `/dev/mali0`(10,125), `/dev/media0`(250,0), `/dev/video0`(81,3), `/dev/drp1`(234,1) (Source: 00_inventory.txt:119-127; ✅ re-run on-board 2026-07-17, all six major/minor numbers matched, transcript: live/ch04-inventory.txt). To further confirm driver versions, check the boot messages:
...
**Memory specs and one important "why."** The board has 16 GB of LPDDR4X-3200 (2×32-bit, theoretical peak 25.6 GB/s), no swap; `meminfo` measures `MemTotal 15565904 kB`, `MemAvailable 14863188 kB` (Source: 00_inventory.txt:90-97). Note: **the bandwidth the CPU side can actually pull (STREAM 5.6 GB/s) is only about 22% of the memory controller's peak** (Source: 05-compute-benchmark.md:107-109).
...
**The speedup ratio and what it really means.** DRP-AI3 is **9.5×** faster than the CPU at 4 threads, and **27.7×** faster than 1 thread (Source: 05-compute-benchmark.md:145). But more important than "how many times faster" is this: it turns YOLOX inference from 145 ms that eats up all four A55 cores into 15 ms on a single dedicated NPU, **freeing the four A55 cores completely to run control and communication protocols (flight control as an example)** — this is exactly what makes "detect/track while flying" possible in the first place (Source: 05-compute-benchmark.md:147).
...
**Hands-on: enabling OpenCL** (install steps 📼 per the recording; verification commands ✅ re-run on-board 2026-07-17, transcript: live/ch04-followup.txt [clinfo], live/ch04-cpu-periph.txt [eglinfo]; Source: 08-gpu-deep-dive.md:73-75):
...
The first two lines register libmali.so as the OpenCL ICD vendor (installation is a state-changing operation, not re-run during the recheck; already installed on the board), and the third line verifies it took effect. Expect it to list `ARM Platform / Mali-G31 r0p0 / OpenCL 3.0` (✅ re-run on-board 2026-07-17 `clinfo`, transcript: live/ch04-followup.txt: saw verbatim `Platform Name  ARM Platform`, `Device Name  Mali-G31 r0p0`, `Platform Version  OpenCL 3.0 v1.r54p1-…`; also see 08-gpu-deep-dive.md:75, 07-hardware-unit-usage-guide.md:97). To confirm the renderer, use `eglinfo | grep -i renderer`, expecting to print `OpenGL ES profile renderer: Mali-G31` (✅ re-run on-board 2026-07-17, transcript: live/ch04-cpu-periph.txt).
...
|---|---|---|---|---|
| **d04** (2026-06-21, with time-series CSV) | 35 / 36 °C (tz0/tz1) | 38 / 39 °C, all cores, 5 min | 1700 MHz throughout, 548/548 samples, zero throttling | 05-compute-benchmark.md:180-182, 08_thermal.txt:8-10 |
| **d03** (no measurement date/host given) | ~35 °C (room temp 26 °C) | ~50 °C after 5 min of stress-ng; ~42 °C under continuous YOLOX inference | No throttling, no reboot | 04-hardware-quickref.md:210-212 |

**The two measurements' peaks differ a lot, yet both say no throttling — this has to be laid out honestly side by side, not cherry-picked.** d04's peak under full load is 38–39 °C, while d03's peak after 5 minutes of stress-ng reaches roughly 50 °C (Source: 04-hardware-quickref.md:210-212, 05-compute-benchmark.md:180-182). The gap comes from different measurement conditions (load type, environment, whether time-series data was attached) — d04 has point-by-point CSV evidence, d03 comes with no date/host given. The shared conclusion is still reliable: **fanless, zero throttling, only a few degrees of temperature rise under full load**; the board has a metal heatsink and needs no active cooling for now — this also echoes the DRP-AI3 white paper's claim that "fanless alone reaches the class of fan-equipped competitors" (Source: 05-compute-benchmark.md:184-185, 04-hardware-quickref.md:214).
...
**Finally, let's nail down the ceiling for Linux hard real-time**: worst-case jitter under load is 926 µs; if jitter should account for <10% of the period, the period needs to be >9.26 ms (~108 Hz); with a 3× safety margin, that's roughly 360 Hz. So the **reliable hard-loop ceiling on Linux/A55 is roughly 100–360 Hz**; anything requiring a guaranteed hard deadline >300–500 Hz (inner-loop attitude/RPM, motor commutation) **must run on the Cortex-R8** (Source: 09-compute-capability.md:35). This point, the earlier cyclictest data, and R8's role all confirm each other — it's the one sentence in this section most worth remembering.
...
> ⚠️ **Warning**: **Scenario** — you expect any DRP-AI3 graph to squeeze under 10 ms of inference. **Symptom** — even a tiny model can't get there. **Cause** — pre-processing (~7 ms) + post-processing (~1.6 ms) (both on A55) + a fixed NPU startup/DMA floor of roughly 8–10 ms; a small model doesn't get proportionally faster in line with its FLOPs. **Prevention** — when scheduling, always budget end-to-end time as "greater than the inference time itself" (measured end-to-end is 23.7 ms vs. pure inference 15.3 ms) — don't estimate overall latency from inference time alone (Source: 09-compute-capability.md:78).
...
**✅ Verification 1 (re-run on-board 2026-07-17, transcript: live/ch04-cpu-periph.txt): Is the clock and governor locked correctly?**
...
**✅ Verification 2 (re-run on-board 2026-07-17, confirmed no output, transcript: live/ch04-followup.txt): Are the crypto extensions really absent?**
...
**⏸ Verification 3 (benchmark load, occupied by live services, not re-run — per the recording; requires `sudo apt install sysbench` first, since a clean image doesn't have it pre-installed and you'll otherwise get command not found): Run a sysbench and see whether multi-core scaling is close to linear.**
...
**✅ Verification 4 (re-run on-board 2026-07-17, see note): Confirm thermals are normal and there's no throttling.**
...
Pass/fail: at idle, both zones should read around 35000–36000 (i.e., 35–36 °C) (units of m°C). After 5 minutes of full load, they should still sit well below the throttling threshold — this section measured 1700 MHz throughout, 548/548 samples, zero throttling (Source: 00_inventory.txt:156-157, 05-compute-benchmark.md:180-182). **Note: these are bare-board numbers; you must re-measure once it's in an enclosure.** (✅ note from re-running on-board 2026-07-17, transcript: live/ch04-cpu-periph.txt [temperature], live/ch04-reserved-mem.txt [load average]: at that time both zones read `39000` (39 °C) — the board had resident services running, load average around 1.0, so it was not actually idle; running a few degrees above the 35–36 °C idle baseline is reasonable under those conditions. To check the "35–36 °C idle" figure properly, measure with no load.)

**⏸ Verification 5 (requires SCHED_FIFO and a full-load scenario, occupied by live services, not re-run — per the recording; optional, requires rt-tests).**
