...
| [PMU (PCU + PWC)](#3-pmu-pcu--pwc-power-management) | The power-management sub-unit inside the CPG; handles power-domain isolation and external power/reset sequencing | Enabled; `/sys/power/state` offers `freeze mem disk`, but **there is no usable wake source after an s2idle sleep (on-board finding — see the warning box in that section) — don't suspend a headless board** |
...
How the interrupt number space is carved up is worth remembering up front, because it directly determines "how many interrupt numbers Linux (the CA55 side) gets." The numbering space for **SPI (Shared Peripheral Interrupt)** (r01uh1032 Table 1.5-2, p141): 0–251 are shared across all three CPUs (Common); 252–352 are each CPU's own SYSTEM class (SYSTEM(CA55)/SYSTEM(CM33)/SYSTEM(CR8)); 353–479 are the SINGLE class for CA55 and the SELECT class (software-selectable) for CM33/CR8; 480–959 exist only for CA55 (SINGLE) and are not applicable (—) to CM33/CR8. In other words, **CA55 (the Linux side) gets a much larger interrupt-number space than CM33/CR8 do**.
...
- **GIC-600 register block base** = `0x14900000` (Cortex-A55 address space; CM33 secure `0x44900000`, non-secure `0x54900000`). **Address correction note**: doc07 §10 transcribes this, along with the GICR, as `0x14800000`/`0x14840000`, but `0x14800000` is actually the SRAM2(REG) region per §1.8 Address Map. That transcription is in error; the value given here is the corrected one, taken from the official Manual §1.8 (p167) and §4.6.2.2 (p961): `0x14900000`. This section of the Manual only gives the block base address; the individual GICD/GICR frame offsets point to the Arm GIC-600 TRM (Technical Reference Manual), so the Manual doesn't separately list the sub-frame addresses.
...
> ⚠️ **Note (never suspend a headless board — on this board, s2idle is a one-way trip)**:
> - **Situation**: To save power, you send the board `echo freeze > /sys/power/state` or `rtcwake -m freeze -s N`, expecting it to sleep for a while and wake up on its own.
> - **Symptom**: Once the board enters s2idle (suspend-to-idle — only processes and I/O are frozen, the CPU goes into a shallow idle) it **never wakes again** — the network drops, SSH drops, and the serial console gets no response to any keypress. On this board, `rtcwake -m freeze -s 45` freezes it and takes it off the network, the serial port reads back 0 bytes, and only a **physical power cycle** brings it back (on-board finding; transcript: `live/ch4-pmu-suspend.txt`).
> - **Cause**: `/sys/power/state` does list `freeze mem disk`, and `rtcwake -m no` **can** set an RTC alarm (the alarm ioctl on `/dev/rtc0` works), but on this board the **RTC alarm interrupt is not wired up as an s2idle wake source** — the alarm coming due doesn't wake the system, and the serial UART isn't an enabled wake source either. **"The alarm can be set" is not the same as "it can wake up."**
> - **Prevention**: **Never enter suspend on a remote/headless setup.** If you genuinely need to validate low power, first make sure there is a wake source that **will actually fire** (a physical wake signal wired up, for instance, or the AWO mode reached via the CM33 boot path), and do it **with a person on site who can cut power**. For power savings, reach first for CPU governor frequency scaling (see [01-compute-units.md](01-compute-units.md)) plus gating the clocks of peripherals you don't use, rather than whole-machine suspend.
...
- **The PMU has no independent APB address space** — it's controlled by the CPG's registers (CPG base `0x1042_0000`). The Manual explicitly states on p790, "PMU is a unit in the CPG," a point transcribed via doc07 §17.
...
### Hands-On: Read Out the Sleep States This Board Supports, and Try Suspend Safely

**Mechanism**: Linux abstracts "how deep do you want to sleep" into a few keywords in `/sys/power/state` — `freeze` (suspend-to-idle: only processes and I/O are frozen, the CPU goes into a shallow idle), `mem` (suspend-to-RAM: the platform powers down most power domains, with memory self-refresh preserving its contents), and `disk` (hibernate: memory is written to disk and the machine powers off). But "how deep `mem` actually sleeps" further depends on `/sys/power/mem_sleep`: it lists the implementations available behind `mem`, with the currently selected one wrapped in square brackets `[...]`. The point of this step is to establish **which sleep states this board actually offers** before deciding whether it's safe to really go to sleep.

**Step 1: read out the list of supported states (read-only, safe).** (✅ Verified on the board; transcript: `live/ch4-w1-pmu.txt`)
...
Read those two lines together and you get a far more honest picture than `state` alone: `state` does list `freeze mem disk`, but `mem_sleep` offers only `[s2idle]` and no `deep` — meaning **on this board, `mem` is backed by s2idle (suspend-to-idle), and there is no platform-level deep suspend-to-RAM**. In other words, `freeze` and `mem` take the same shallow sleep path here; `disk` (hibernate) additionally needs a resume device (a swap partition or file) configured before it really works, and this board has none set up by default.

Two more read-only counters are worth reading while you're here, since they help you interpret the state before and after a suspend (same transcript):
...
`wakeup_count` is the running total of wake events so far; `pm_freeze_timeout` (in milliseconds) is the timeout for waiting on processes to freeze. To look further into each power domain (genpd), `pm_genpd` lives under debugfs and needs root:

```bash
sudo cat /sys/kernel/debug/pm_genpd/pm_genpd_summary
```

> 💡 **Tip**: Reading it without `sudo` is blocked — as an ordinary user, `ls /sys/kernel/debug/pm_genpd` returns `Permission denied` (transcript: `live/ch4-w1-pmu.txt`). debugfs is root-only by default; that's normal permissions, not something broken.

**Step 2: actually going to sleep — ⏸ not demonstrated on this board; here's the safe procedure instead.**

Entering suspend takes one line (s2idle as the example):
...
# or a local terminal, never over SSH alone
sudo rtcwake -m freeze -s 30
```

`rtcwake` sets the RTC alarm first and then enters the requested sleep state, so the hardware wakes the board when the time comes — it comes back on its own even if SSH has dropped. Note, however, the warning box earlier in this section: on this board the RTC alarm has been found **not** to fire as an s2idle wake source, so treat this path as unverified and attempt it only **during a maintenance window with physical access to the board**. When it does work, `dmesg` shows the pair `PM: suspend entry (s2idle)` → `PM: suspend exit`.

**Pass criteria**:
- What Step 1 looks like when done right: `state` contains at least `freeze`; and whatever `mem_sleep` wraps in `[...]` is the implementation `mem` will actually use (`s2idle` on this board).
- If you do go on to Step 2: note `cat /sys/power/wakeup_count` before sleeping and read it again after waking — an **increase** means a wake event really did occur; `dmesg | grep -i "PM: suspend"` should show a matching entry/exit pair.

> ⚠️ **Note (going to sleep remotely can mean never waking up, which is the same as a hang)**
> - **Situation**: You SSH into the board and run `echo mem > /sys/power/state` straight away to try out sleep.
> - **Symptom**: SSH stops responding the instant the command goes out, and afterwards nothing can connect — the board looks hung.
> - **Cause**: Network interfaces are disabled once suspend is entered, and with no wake source arranged in advance (RTC alarm, a physical wake pin, a usable button), the system has no event to wake itself with.
> - **Prevention/Handling**: Always arrange an automatic wake with `rtcwake -s <seconds>` before sleeping; do the first attempt next to the serial console (CN8), with the ability to power-cycle the board in reach — never over SSH alone.
