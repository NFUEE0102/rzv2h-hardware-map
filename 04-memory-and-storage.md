...
Let's start with a diagram that puts these five units into one shared frame of reference, so you won't lose your bearings as you read each section below. The top two blocks are on-chip and the other three are off-chip; the left half is the path for "used for execution / as working memory," and the right half is the path for "used for long-term data storage":
...
```text
  ...
  └───────────────────────────────────────────────────────────────────────────┘
                                     │
  ┌─────────────────────────── External (off-chip) ───────────────────────────┐
  │  LPDDR4/4X controller ×2  →  16 GB LPDDR4 (volatile main memory)           │
...
| [LPDDR4/4X Memory Controller ×2](#lpddr44x-memory-controller-2) | Dual-channel external DRAM controller, supporting LPDDR4/4X-3200 and in-line ECC — the sole path to the system's main memory | Enabled (16 GB LPDDR4/4X-3200, 1600 MHz = 3200 MT/s, initialized by BL2, ~15.5 GB available to Linux) |
...
This board uses SRAM0/1 as the cold-boot landing area for CM33, with the remaining blocks used by remoteproc's vring — that's just **one instance** of SRAM's general role of "hosting heterogeneous-core boot and IPC," not the only way SRAM can be used. Swap in a different firmware set, or a different division of labor between cores, and the blocks it lands on will differ.
...
- **Bandwidth has to be counted across both controllers (don't just take the 32-bit figure from the line above)**: this SoC has **two** memory controllers, DDR0 and DDR1, 32-bit each (the Manual names every register `DDR_MEMCm_*`, m = 0/1, i.e. one set per MC), for a combined 64-bit bus. So the theoretical peak = 2 × (3200 Mbps × 32-bit ÷ 8) = 2 × 12.8 = **25.6 GB/s**; apply 3200 Mbps to the "32-bit" from the line above on its own and you get only 12.8 GB/s (exactly half — not an arithmetic slip in the book). The first-hand hardware manual `r01uh1032` §1.1's functional-overview diagram, Figure 1.1-1 (p77), writes it verbatim as "32 bits × 2 (12.8 GB/s × 2)" — which is where the 25.6 GB/s used consistently throughout this handbook comes from (4.2's STREAM measurement of about 22% is built on this same peak).
...
- **On-board measurement (doc07 §48, Source: `07-hardware-unit-usage-guide.md:797-809`)**: 16 GB of LPDDR4/4X-3200 is fitted (the board's headline spec table says LPDDR4 1600 MHz while the board block diagram labels it LPDDR4X-3200; the controller supports both), with about 15.5 GB available to Linux; 25.6 GB/s is the theoretical peak bandwidth (doc07's original text explicitly marks it "not measured"); both channels are initialized by the boot firmware BL2; DDR0/DDR1's primary regions are mapped at `0x40000000` and `0x140000000` respectively (8 GB each, quoting doc07's citation of the Manual's Table 1.8-1).
- **The capacity is this board's configuration, not a spec ceiling (a 7-2 reminder)**: 16 GB is the capacity this specific board actually has soldered on and initialized, spread across the two controllers DDR0/DDR1 (8 GB each — see the on-board measurement bullet above). That is within spec (up to 8 GB per channel on each controller), but it **does not reach the spec ceiling**: the two controllers together can go up to 2 × 16 = 32 GB, and this board is fitted with half of that. Solder a different capacity onto another board, and it only affects "how much data you can store" — it **does not affect the controller's own capability** — the rate, ECC, and arbitration mechanism all stay the same. The WS125 board manual's spec table (`ws125-rdk-board-manual.md` Table 1) lists "Memory LPDDR4 1600MHz (8GB) x2," which cross-confirms doc07's measured 16 GB (8 GB×2) — but that's only "how much this particular board is fitted with"; it can't be generalized into "the RZ/V2H can only take 16 GB."
...
This board's NOR flash is sliced into four MTD partitions, each with a clear purpose (partition names verbatim, verified on the board with `cat /proc/mtd`; transcript: live/ch04-inventory.txt; Source: `07-hardware-unit-usage-guide.md:769-781`, `04-hardware-quickref.md:278`):
...
This board's actual status (doc07 §47, Source: `07-hardware-unit-usage-guide.md:783-795`): `SDHI0 @0x15C00000` carries rootfs on `/dev/mmcblk0` (a 256 GB Samsung SD card); the other two eMMC controllers, `@0x15C10000` and `@0x15C20000`, are disabled in the device tree — because this particular board doesn't have physical eMMC populated (soldered on/plugged in). So this handbook's resource map marks this unit's overall status as "Partially Enabled": SDHI0 is alive, the two eMMC controllers are switched off (verified on the board: `mmc@15c10000`/`mmc@15c20000` are both `disabled`; transcript: live/ch04b-dt-status.txt).
...
The identity of this board's boot card (Source: `05-compute-benchmark.md:48`, `04b_emmc_id.txt:2-8`): type SD, manfid `0x1b`, oemid `"SM"`, manufactured 2025/10, ext4-formatted, about 206 GB usable. Its speed sits at the UHS-I SD card tier, not the 150-300 MB/s of eMMC 5.1. The full storage I/O benchmarks (sequential read/write, fio random read/write, etc.) are in this handbook's 4.2, "Storage I/O," and aren't repeated here.
...
> - **Prevention/Handling**: to determine what the boot storage actually is, don't trust the script's filenames — read the card's CID (Card Identification register — it holds the manufacturer ID, product name, and manufacturing date) and type instead; to confirm its speed tier, look at which transfer mode it's running in (UHS-I/SDR104/DDR50), not its capacity.
